...
public:
    ...
    static bool HandleGMVisibleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_YOU_ARE, handler->GetSession()->GetPlayer()->isGMVisible() ? handler->GetInfinityString(LANG_VISIBLE) : handler->GetInfinityString(LANG_INVISIBLE));
            return true;
        }
