...
*/
    ...
    static bool HandleDebugSendOpcodeCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        while (!parsedStream.eof())
        {
            ...
            {
                IC_LOG_ERROR("misc", "Sending opcode that has unknown type '%s'", type.c_str());
                break;
            }
        }
        IC_LOG_DEBUG("network", "Sending opcode %u", data.GetOpcode());
        data.hexlike();
    ...
    static bool HandleWPGPSCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        IC_LOG_INFO("sql.dev", "(@PATH, XX, %.3f, %.3f, %.5f, 0, 0, 0, 100, 0),", player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
