...
SpellCastResult SpellInfo::CheckShapeshift(uint32 form) const
{
    ...
    if (form > 0)
    {
        ...
        if (!shapeInfo)
        {
            IC_LOG_ERROR("spells", "GetErrorAtShapeshiftedCast: unknown shapeshift %u", form);
            return SPELL_CAST_OK;
        }
        ...
        if (caster != unitTarget)
        {
            if (caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (Mechanic == MECHANIC_DISARM)
                {
                    ...
                    else if (!unitTarget->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY))
                        return SPELL_FAILED_TARGET_NO_WEAPONS;
                }
            }
        }
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask) const
{
    ...
    if (AttributesEx & SPELL_ATTR1_DRAIN_ALL_POWER)
    {
        ...
        IC_LOG_ERROR("spells", "SpellInfo::CalcPowerCost: Unknown power type '%d' in spell %d", PowerType, Id);
        return 0;
    }
    ...
    if (ManaCostPercentage)
    {
        switch (PowerType)
        {
            ...
            case POWER_HAPPINESS:
                ...
                break;
            default:
                IC_LOG_ERROR("spells", "CalculateManaCost: Unknown power type '%d' in spell %d", PowerType, Id);
                return 0;
        }
    }
