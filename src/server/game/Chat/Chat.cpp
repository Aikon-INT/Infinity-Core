...
{
    const char *format = GetInfinityString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetInfinityString(int32 entry) const
{
    return m_session->GetInfinityString(entry);
}
...
{
    SendSysMessage(GetInfinityString(entry));
}
...
bool ChatHandler::SetDataForCommandInTable(ChatCommand* table, char const* text, uint32 permission, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; table[i].Name != NULL; i++)
    {
        ...
        {
            IC_LOG_ERROR("sql.sql", "Table `command` have unexpected subcommand '%s' in command '%s', skip.", text, fullcommand.c_str());
            return false;
        }

        if (table[i].Permission != permission)
            IC_LOG_INFO("misc", "Table `command` overwrite for command '%s' default permission (%u) by %u", fullcommand.c_str(), table[i].Permission, permission);
    ...
    if (!cmd.empty())
    {
        if (table == getCommandTable())
            IC_LOG_ERROR("sql.sql", "Table `command` have not existed command '%s', skip.", cmd.c_str());
        else
            IC_LOG_ERROR("sql.sql", "Table `command` have not existed subcommand '%s' in command '%s', skip.", cmd.c_str(), fullcommand.c_str());
    }
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    Infinity::NearestGameObjectCheck check(*pl);
    Infinity::GameObjectLastSearcher<Infinity::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(Infinity::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        Infinity::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        Infinity::GameObjectSearcher<Infinity::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<Infinity::GameObjectSearcher<Infinity::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetInfinityString(int32 entry) const
{
...
{
    return GetInfinityString(LANG_CONSOLE_COMMAND);
}
