...
#ifndef INFINITY_OBJECTDEFINES_H
#define INFINITY_OBJECTDEFINES_H
...
inline bool IS_CRE_GUID(uint64 guid);
...
bool IS_CRE_GUID(uint64 guid)
{
    return IS_CREATURE_GUID(guid);
}

bool IS_CRE_OR_PET_GUID(uint64 guid)
{
    return IS_CRE_GUID(guid) || IS_PET_GUID(guid);
}
