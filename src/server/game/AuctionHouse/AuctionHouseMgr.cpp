...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    IC_LOG_DEBUG("auctionHouse", "MSV:        %u", MSV);
    IC_LOG_DEBUG("auctionHouse", "Items:      %u", count);
    IC_LOG_DEBUG("auctionHouse", "Multiplier: %f", multiplier);
    IC_LOG_DEBUG("auctionHouse", "Deposit:    %u", deposit);
...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, SQLTransaction& trans)
{
    ...
    else
    {
        ...
        if (logGmTrade && !sObjectMgr->GetPlayerNameByGUID(bidderGuid, bidderName))
            bidderName = sObjectMgr->GetInfinityStringForDBCLocale(LANG_UNKNOWN);
    }
    ...
    if (logGmTrade)
    {
        ...
        if (!sObjectMgr->GetPlayerNameByGUID(auction->owner, ownerName))
            ownerName = sObjectMgr->GetInfinityStringForDBCLocale(LANG_UNKNOWN);
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        IC_LOG_INFO("server.loading", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    do
    {
        ...
        if (!proto)
        {
            IC_LOG_ERROR("misc", "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u id: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
    ...
    IC_LOG_INFO("server.loading", ">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        IC_LOG_INFO("server.loading", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
    ...
    IC_LOG_INFO("server.loading", ">> Loaded %u auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item)
    {
        IC_LOG_ERROR("misc", "AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        IC_LOG_ERROR("misc", "Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        IC_LOG_ERROR("misc", "Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    {
        IC_LOG_ERROR("misc", "Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
    ...
    {
        IC_LOG_ERROR("misc", "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
...
void AuctionHouseMgr::DeleteExpiredAuctionsAtStartup()
{
    ...
    if (!expAuctions)
    {
        IC_LOG_INFO("server.loading", ">> No expired auctions to delete");
    ...
    IC_LOG_INFO("server.loading", ">> Deleted %u expired auctions in %u ms", expirecount, GetMSTimeDiffToNow(oldMSTime));
...
bool AuctionEntry::LoadFromFieldList(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        IC_LOG_ERROR("misc", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        IC_LOG_ERROR("misc", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    if (!auctionHouseEntry)
    {
        IC_LOG_ERROR("misc", "AuctionEntry::LoadFromFieldList() - Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
