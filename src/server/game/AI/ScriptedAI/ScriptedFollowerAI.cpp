...
void FollowerAI::EnterEvadeMode()
{
    ...
    {
        IC_LOG_DEBUG("scripts", "FollowerAI left combat, returning to CombatStartPosition.");
...
void FollowerAI::UpdateAI(uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->GetVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            {
                IC_LOG_DEBUG("scripts", "FollowerAI is set completed, despawns.");
            ...
            if (Player* player = GetLeaderForFollower())
            {
                ...
                {
                    IC_LOG_DEBUG("scripts", "FollowerAI is returning to leader.");
            ...
            {
                IC_LOG_DEBUG("scripts", "FollowerAI failed because player/group was to far away or not found");
                me->DespawnOrUnsummon();
                return;
            }
...
void FollowerAI::StartFollow(Player* player, uint32 factionForFollower, const Quest* quest)
{
    if (me->GetVictim())
    {
        IC_LOG_DEBUG("scripts", "FollowerAI attempt to StartFollow while in combat.");
        return;
    }

    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        IC_LOG_ERROR("scripts", "FollowerAI attempt to StartFollow while already following.");
        return;
    }
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        IC_LOG_DEBUG("scripts", "FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
    }
    ...
    IC_LOG_DEBUG("scripts", "FollowerAI start follow %s (GUID " UI64FMTD ")", player->GetName().c_str(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower()
{
        ...
        else
        {
            if (Group* group = player->GetGroup())
            {
                for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
                {
                    ...
                    {
                        IC_LOG_DEBUG("scripts", "FollowerAI GetLeader changed and returned new leader.");
    ...
    IC_LOG_DEBUG("scripts", "FollowerAI GetLeader can not find suitable leader.");
    return NULL;
}
