...
void WorldSession::HandlePetStopAttack(WorldPacket &recvData)
{
    ...
    IC_LOG_DEBUG("network", "WORLD: Received CMSG_PET_STOP_ATTACK for GUID " UI64FMTD "", guid);

    Unit* pet = ObjectAccessor::GetCreatureOrPet(*_player, guid);

    if (!pet)
    {
        IC_LOG_ERROR("network", "HandlePetStopAttack: Pet %u does not exist", uint32(GUID_LOPART(guid)));
        return;
    }

    if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharm())
    {
        IC_LOG_ERROR("network", "HandlePetStopAttack: Pet GUID %u isn't a pet or charmed creature of player %s",
...
{
    IC_LOG_INFO("network", "HandlePetNameQuery. CMSG_PET_NAME_QUERY");
...
bool WorldSession::CheckStableMaster(uint64 guid)
{
    ...
    if (guid == GetPlayer()->GetGUID())
    {
        ...
        {
            IC_LOG_DEBUG("network", "Player (GUID:%u) attempt open stable in cheating way.", GUID_LOPART(guid));
            return false;
        }
    }
    // stable master case
    else
    {
        ...
        {
            IC_LOG_DEBUG("network", "Stablemaster (GUID:%u) not found or you can't interact with him.", GUID_LOPART(guid));
            return false;
        }
    }
    return true;
...
{
    IC_LOG_INFO("network", "HandlePetSetAction. CMSG_PET_SET_ACTION");
    ...
    {
        IC_LOG_ERROR("network", "HandlePetSetAction: Unknown pet (GUID: %u) or pet owner (GUID: %u)", GUID_LOPART(petguid), _player->GetGUIDLow());
        return;
    }
    ...
    if (!charmInfo)
    {
        IC_LOG_ERROR("network", "WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        IC_LOG_INFO("network", "Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X",
...
{
    IC_LOG_INFO("network", "HandlePetRename. CMSG_PET_RENAME");
...
void WorldSession::HandlePetAbandon(WorldPacket& recvData)
{
    ...
    IC_LOG_INFO("network", "HandlePetAbandon. CMSG_PET_ABANDON pet guid is %u", GUID_LOPART(guid));
...
{
    IC_LOG_INFO("network", "CMSG_PET_SPELL_AUTOCAST");
    uint64 guid;
    uint32 spellid;
    ...
    {
        IC_LOG_ERROR("network", "HandlePetSpellAutocastOpcode.Pet %u isn't pet of player %s (GUID: %u).", uint32(GUID_LOPART(guid)), GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()));
        return;
    }
    ...
    if (!spellInfo)
    {
        IC_LOG_ERROR("network", "WORLD: unknown PET spell id %u", spellid);
        return;
    }
    ...
    if (!charmInfo)
    {
        IC_LOG_ERROR("network", "WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
...
{
    IC_LOG_DEBUG("network", "WORLD: CMSG_PET_CAST_SPELL");
    ...
    IC_LOG_DEBUG("network", "WORLD: CMSG_PET_CAST_SPELL, guid: " UI64FMTD ", castCount: %u, spellId %u, castFlags %u", guid, castCount, spellId, castFlags);
    ...
    {
        IC_LOG_ERROR("network", "HandlePetCastSpellOpcode: Pet %u isn't pet of player %s (GUID: %u).", uint32(GUID_LOPART(guid)), GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()));
        return;
    }
...
{
    IC_LOG_DEBUG("network", "WORLD: CMSG_PET_LEARN_TALENT");
...
{
    IC_LOG_DEBUG("network", "CMSG_LEARN_PREVIEW_TALENTS_PET");
