...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        IC_LOG_ERROR("scripts", "Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", soundId, source->GetTypeId(), source->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        IC_LOG_ERROR("scripts", "DoResetThreat called for creature that either cannot have threat list or has empty threat list (me entry = %d)", me->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        IC_LOG_ERROR("scripts", "Creature " UI64FMTD " (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: " UI64FMTD ") to x: %f y:%f z: %f o: %f. Aborted.",
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float range)
{
    ...
    Infinity::CreatureListSearcher<Infinity::FriendlyCCedInRange> searcher(me, list, u_check);
    me->VisitNearbyObject(range, searcher);
...
{
    std::list<Creature*> list;
    Infinity::FriendlyMissingBuffInRange u_check(me, range, uiSpellid);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float minimumRange)
{
    ...
    CellCoord pair(Infinity::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    Infinity::PlayerAtMinimumRangeAway check(me, minimumRange);
    Infinity::PlayerSearcher<Infinity::PlayerAtMinimumRangeAway> searcher(me, player, check);
    TypeContainerVisitor<Infinity::PlayerSearcher<Infinity::PlayerAtMinimumRangeAway>, GridTypeMapContainer> visitor(searcher);
...
void ScriptedAI::SetEquipmentSlots(bool loadDefault, int32 mainHand /*= EQUIP_NO_CHANGE*/, int32 offHand /*= EQUIP_NO_CHANGE*/, int32 ranged /*= EQUIP_NO_CHANGE*/)
{
    ...
    if (mainHand >= 0)
        me->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY + 0, uint32(mainHand));

    if (offHand >= 0)
        me->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY + 1, uint32(offHand));

    if (ranged >= 0)
        me->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY + 2, uint32(ranged));
}
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(uint32 const diff)
{
    ...
    switch (me->GetEntry())
    {
        ...
        default: // For most of creatures that certain area is their home area.
            IC_LOG_INFO("misc", "TSCR: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition. Using the default one.", me->GetEntry());
