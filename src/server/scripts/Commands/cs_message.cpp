...
public:
    ...
    static bool HandleAnnounceCommand(ChatHandler* handler, char const* args)
    {
        ...
        char buff[2048];
        sprintf(buff, handler->GetInfinityString(LANG_SYSTEMMESSAGE), args);
    ...
    static bool HandleNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetInfinityString(LANG_GLOBAL_NOTIFY);
        str += args;
    ...
    static bool HandleGMNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetInfinityString(LANG_GM_NOTIFY);
        str += args;
    ...
    static bool HandleWhispersCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, handler->GetSession()->GetPlayer()->isAcceptWhispers() ?  handler->GetInfinityString(LANG_ON) : handler->GetInfinityString(LANG_OFF));
            return true;
        }
