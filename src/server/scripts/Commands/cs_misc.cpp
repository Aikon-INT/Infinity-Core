...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        CellCoord cellCoord = Infinity::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
        ...
        GridCoord gridCoord = Infinity::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetInfinityString(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target == handler->GetSession()->GetPlayer() ? handler->GetInfinityString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->RemoveSpellCooldown(spellIid, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, target == handler->GetSession()->GetPlayer() ? handler->GetInfinityString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetInfinityString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetInfinityString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetInfinityString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetInfinityString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        IC_LOG_DEBUG("misc", handler->GetInfinityString(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        IC_LOG_DEBUG("misc", handler->GetInfinityString(LANG_ADDITEMSET), itemSetId);
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string userName          = handler->GetInfinityString(LANG_ERROR);
        uint32 accId                  = 0;
        uint32 lowguid                = GUID_LOPART(targetGuid);
        std::string eMail             = handler->GetInfinityString(LANG_ERROR);
        std::string regMail           = handler->GetInfinityString(LANG_ERROR);
        uint32 security               = 0;
        std::string lastIp            = handler->GetInfinityString(LANG_ERROR);
        uint8 locked                  = 0;
        std::string lastLogin         = handler->GetInfinityString(LANG_ERROR);
        ...
        std::string genderStr           = handler->GetInfinityString(LANG_ERROR);
        uint32 totalPlayerTime          = 0;
        uint8 level                     = 0;
        std::string alive               = handler->GetInfinityString(LANG_ERROR);
        uint32 money                    = 0;
        uint32 xp                       = 0;
        uint32 xptotal                  = 0;
        ...
        handler->PSendSysMessage(LANG_PINFO_PLAYER, target ? "" : handler->GetInfinityString(LANG_OFFLINE), nameLink.c_str(), lowguid);
        ...
        handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetInfinityString(LANG_CHARACTER_GENDER_MALE) : handler->GetInfinityString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(), classStr.c_str());
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        Infinity::RespawnDo u_do;
        Infinity::WorldObjectWorker<Infinity::RespawnDo> worker(player, u_do);

        TypeContainerVisitor<Infinity::WorldObjectWorker<Infinity::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
