...
class BattlegroundQueue
{
    public:
        ...
        void BattlegroundQueueUpdate(uint32 diff, BattlegroundTypeId bgTypeId, uint8 arenaType = 0, bool isRated = false, uint32 minRating = 0);
        ...
        GroupQueueInfo* AddGroup(Player* leader, Group* group, BattlegroundTypeId bgTypeId, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 ArenaTeamId = 0);
    ...
    private:
        ...
        uint32 m_WaitTimes[BG_TEAMS_COUNT][COUNT_OF_PLAYERS_TO_AVERAGE_WAIT_TIME];
        uint32 m_WaitTimeLastPlayer[BG_TEAMS_COUNT];
        uint32 m_SumOfWaitTimes[BG_TEAMS_COUNT];
