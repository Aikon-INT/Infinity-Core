...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    if (cinfo->type == CREATURE_TYPE_CRITTER)
    {
        ...
        if (!IsPositionValid())
        {
            IC_LOG_ERROR("entities.pet", "Pet (guidlow %d, entry %d) not loaded. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    switch (getPetType())
    {
        ...
        case HUNTER_PET:
            ...
            break;
        default:
                IC_LOG_ERROR("entities.pet", "Pet have incorrect type (%u) for pet loading.", getPetType());
            break;
    }
    ...
    IC_LOG_DEBUG("entities.pet", "New Pet has guid %u", GetGUIDLow());
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    IC_LOG_ERROR("entities.pet", "Pet %u is not pet of owner %s, removed", GetEntry(), GetOwner()->GetName().c_str());
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    if (!IsPositionValid())
    {
        IC_LOG_ERROR("entities.pet", "Pet (guidlow %d, entry %d) not created base at creature. Suggested coordinates isn't valid (X: %f Y: %f)",
            GetGUIDLow(), GetEntry(), GetPositionX(), GetPositionY());
        return false;
    }
    ...
    if (!cinfo)
    {
        IC_LOG_ERROR("entities.pet", "CreateBaseAtCreature() failed, creatureInfo is missing!");
        return false;
    }
...
{
    IC_LOG_DEBUG("entities.pet", "Pet::CreateBaseForTamed");
    uint32 guid=sObjectMgr->GenerateLowGuid(HIGHGUID_PET);
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (IsPet() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
    {
        if (GetOwner()->getClass() == CLASS_WARLOCK
                || GetOwner()->getClass() == CLASS_SHAMAN)
        {
            petType = SUMMON_PET;
        }
        ...
        {
            IC_LOG_ERROR("entities.pet", "Unknown type pet %u is summoned by player class %u",
...
void Pet::_LoadSpellCooldowns()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                IC_LOG_ERROR("entities.pet", "Pet %u have unknown spell %u in `pet_spell_cooldown`, skipping.", m_charmInfo->GetPetNumber(), spell_id);
                continue;
            }
            ...
            IC_LOG_DEBUG("entities.pet", "Pet (Number: %u) spell %u cooldown loaded (%u secs).", m_charmInfo->GetPetNumber(), spell_id, uint32(db_time-curTime));
        }
...
{
    IC_LOG_DEBUG("entities.pet", "Loading auras for pet %u", GetGUIDLow());
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                IC_LOG_ERROR("entities.pet", "Unknown aura (spellid %u), ignore.", spellid);
                continue;
            }
            ...
            if (Aura* aura = Aura::TryCreate(spellInfo, effmask, this, NULL, &baseDamage[0], NULL, caster_guid))
            {
                ...
                IC_LOG_INFO("entities.pet", "Added aura spellid %u, effectmask %u", spellInfo->Id, effmask);
            }
        }
        while (result->NextRow());
    }
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            IC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #%u request, deleting for all pets in `pet_spell`.", spellId);
            ...
            IC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #%u request.", spellId);
