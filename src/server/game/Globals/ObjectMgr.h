...
// Infinity string ranges
#define MIN_INFINITY_STRING_ID           1                    // 'infinity_string'
#define MAX_INFINITY_STRING_ID           2000000000
#define MIN_DB_SCRIPT_STRING_ID        MAX_INFINITY_STRING_ID // 'db_script_string'
#define MAX_DB_SCRIPT_STRING_ID        2000010000
#define MIN_CREATURE_AI_TEXT_STRING_ID (-1)                 // 'creature_ai_texts'
#define MAX_CREATURE_AI_TEXT_STRING_ID (-1000000)

// Infinity Trainer Reference start range
#define INFINITY_TRAINER_START_REF      200000

struct InfinityStringLocale
{
...
typedef UNORDERED_MAP<int32, InfinityStringLocale> InfinityStringLocaleContainer;
...
*/
        ...
        {
            IC_LOG_INFO("server.loading", "Loading GO Start Quest Data...");
            LoadGameobjectQuestStarters();
            IC_LOG_INFO("server.loading", "Loading GO End Quest Data...");
            LoadGameobjectQuestEnders();
            IC_LOG_INFO("server.loading", "Loading Creature Start Quest Data...");
            LoadCreatureQuestStarters();
            IC_LOG_INFO("server.loading", "Loading Creature End Quest Data...");
        ...
        bool LoadInfinityStrings(char const* table, int32 min_value, int32 max_value);
        bool LoadInfinityStrings() { return LoadInfinityStrings("infinity_string", MIN_INFINITY_STRING_ID, MAX_INFINITY_STRING_ID); }
        ...
        InfinityStringLocale const* GetInfinityStringLocale(int32 entry) const
        {
            InfinityStringLocaleContainer::const_iterator itr = _trinityStringLocaleStore.find(entry);
            if (itr == _trinityStringLocaleStore.end()) return NULL;
            return &itr->second;
        }
        const char *GetInfinityString(int32 entry, LocaleConstant locale_idx) const;
        const char *GetInfinityStringForDBCLocale(int32 entry) const { return GetInfinityString(entry, DBCLocaleIndex); }
    ...
    private:
        ...
        InfinityStringLocaleContainer _trinityStringLocaleStore;
        ...
        std::set<uint32> _difficultyEntries[TOTAL_DIFFICULTIES - 1]; // already loaded difficulty 1 value in creatures, used in CheckCreatureTemplate
        std::set<uint32> _hasDifficultyEntries[TOTAL_DIFFICULTIES - 1]; // already loaded creatures with difficulty 1 values, used in CheckCreatureTemplate
...
bool LoadInfinityStrings(char const* table, int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
