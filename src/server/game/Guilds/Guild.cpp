...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param)
{
    ...
    IC_LOG_DEBUG("guild", "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s"
...
void Guild::SendSaveEmblemResult(WorldSession* session, GuildEmblemError errCode)
{
    ...
    IC_LOG_DEBUG("guild", "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u", session->GetPlayerInfo().c_str(), errCode);
}
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, SQLTransaction& trans, bool logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i)
    {
        ...
        if (logOnCreate)
            IC_LOG_ERROR("guild", "Guild %u has broken Tab %u for rank %u. Created default tab.", m_guildId, i, m_rankId);
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    {
        IC_LOG_ERROR("guild", "Invalid slot for item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    if (!proto)
    {
        IC_LOG_ERROR("guild", "Unknown item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    {
        IC_LOG_ERROR("guild", "Item (GUID %u, id: %u) not found in item_instance, deleting from guild bank!", itemGuid, itemEntry);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    if (session)
    {
        IC_LOG_DEBUG("guild", "MSG_QUERY_GUILD_BANK_TEXT [%s]: Tabid: %u, Text: %s"
            , session->GetPlayerInfo().c_str(), m_tabId, m_text.c_str());
        session->SendPacket(&data);
    }
    else
    {
        IC_LOG_DEBUG("guild", "MSG_QUERY_GUILD_BANK_TEXT [Broadcast]: Tabid: %u, Text: %s", m_tabId, m_text.c_str());
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId)
    {
        IC_LOG_ERROR("guild", "Player (GUID: %u) has broken zone-data", GUID_LOPART(m_guid));
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1)
    {
        IC_LOG_ERROR("guild", "Player (GUID: %u) has a broken data in field `characters`.`level`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }

    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES)
    {
        IC_LOG_ERROR("guild", "Player (GUID: %u) has a broken data in field `characters`.`class`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }
    return true;
}
...
Item* Guild::BankMoveItemData::StoreItem(SQLTransaction& trans, Item* pItem)
{
    ...
    for (ItemPosCountVec::const_iterator itr = m_vec.begin(); itr != m_vec.end(); )
    {
        ...
        IC_LOG_DEBUG("guild", "GUILD STORAGE: StoreItem tab = %u, slot = %u, item = %u, count = %u",
...
{
    IC_LOG_DEBUG("guild", "GUILD STORAGE: CanStore() tab = %u, slot = %u, item = %u, count = %u",
        m_container, m_slotId, pItem->GetEntry(), pItem->GetCount());
...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    IC_LOG_DEBUG("guild", "GUILD: creating guild [%s] for leader %s (%u)",
    ...
    if (Member* member = GetMember(player->GetGUID()))
    {
        switch (dataid)
        {
            ...
            case GUILD_MEMBER_DATA_LEVEL:
                ...
                break;
            default:
                IC_LOG_ERROR("guild", "Guild::UpdateMemberData: Called with incorrect DATAID %u (value %u)", dataid, value);
                return;
        }
        //HandleRoster();
    }
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    if (session)
    {
        IC_LOG_DEBUG("guild", "SMSG_GUILD_ROSTER [%s]", session->GetPlayerInfo().c_str());
        session->SendPacket(&data);
    }
    else
    {
        IC_LOG_DEBUG("guild", "SMSG_GUILD_ROSTER [Broadcast]");
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    IC_LOG_DEBUG("guild", "SMSG_GUILD_QUERY_RESPONSE [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::HandleSetBankTabInfo(WorldSession* session, uint8 tabId, std::string const& name, std::string const& icon)
{
    ...
    if (!tab)
    {
        IC_LOG_ERROR("guild", "Guild::HandleSetBankTabInfo: Player %s trying to change bank tab info from unexisting tab %d.",
...
void Guild::HandleSetRankInfo(WorldSession* session, uint8 rankId, std::string const& name, uint32 rights, uint32 moneyPerDay, const GuildBankRightsAndSlotsVec& rightsAndSlots)
{
    ...
    {
        IC_LOG_DEBUG("guild", "Changed RankName to '%s', rights to 0x%08X", name.c_str(), rights);
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    IC_LOG_DEBUG("guild", "Player %s invited %s to join his Guild", player->GetName().c_str(), name.c_str());
    ...
    IC_LOG_DEBUG("guild", "SMSG_GUILD_INVITE [%s]", pInvitee->GetName().c_str());
}
...
void Guild::HandleDisband(WorldSession* session)
{
    ...
    {
        Disband();
        IC_LOG_DEBUG("guild", "Guild Successfully Disbanded");
        delete this;
    }
}
...
void Guild::SendInfo(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    IC_LOG_DEBUG("guild", "SMSG_GUILD_INFO [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    IC_LOG_DEBUG("guild", "MSG_GUILD_EVENT_LOG_QUERY [%s]", session->GetPlayerInfo().c_str());
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        session->SendPacket(&data);
        IC_LOG_DEBUG("guild", "MSG_GUILD_BANK_LOG_QUERY [%s]", session->GetPlayerInfo().c_str());
    }
}
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    IC_LOG_DEBUG("guild", "MSG_GUILD_PERMISSIONS [%s] Rank: %u", session->GetPlayerInfo().c_str(), rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    data << int32(amount);
    session->SendPacket(&data);
    IC_LOG_DEBUG("guild", "MSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: %u", session->GetPlayerInfo().c_str(), amount);
}
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    IC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT [%s] MOTD", session->GetPlayerInfo().c_str());
    ...
    if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->CanInsert())
        {
            ...
            if (BankEventLogEntry::IsMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    IC_LOG_ERROR("guild", "GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: %u) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                IC_LOG_ERROR("guild", "GuildBankEventLog ERROR: non-money event (LogGuid: %u, Guild: %u) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
void Guild::LoadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
        IC_LOG_ERROR("guild", "Invalid tab (tabId: %u) in guild bank, skipped.", tabId);
    else
...
bool Guild::LoadBankItemFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
    {
        IC_LOG_ERROR("guild", "Invalid tab for item (GUID: %u, id: #%u) in guild bank, skipped.",
...
bool Guild::Validate()
{
    ...
    {
        IC_LOG_ERROR("guild", "Guild %u has invalid number of ranks, creating new...", m_id);
        broken_ranks = true;
    }
    else
    {
        for (uint8 rankId = 0; rankId < ranks; ++rankId)
        {
            ...
            {
                IC_LOG_ERROR("guild", "Guild %u has broken rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
            else
            {
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc)
{
    ...
    _CreateRank(sObjectMgr->GetInfinityString(LANG_GUILD_MASTER,   loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetInfinityString(LANG_GUILD_OFFICER,  loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetInfinityString(LANG_GUILD_VETERAN,  loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetInfinityString(LANG_GUILD_MEMBER,   loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetInfinityString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
...
void Guild::_MoveItems(MoveItemData* pSrc, MoveItemData* pDest, uint32 splitedAmount)
{
    ...
    {
        IC_LOG_FATAL("guild", "Guild::SwapItems: Player %s(GUIDLow: %u) tried to move item %u from tab %u slot %u to tab %u slot %u, but item %u has a stack of zero!",
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, uint64 guid, const char* param1, const char* param2, const char* param3) const
{
    ...
    if (sLog->ShouldLog("guild", LOG_LEVEL_DEBUG))
        IC_LOG_DEBUG("guild", "SMSG_GUILD_EVENT [Broadcast] Event: %s (%u)", _GetGuildEventString(guildEvent).c_str(), guildEvent);
}
...
void Guild::_SendBankList(WorldSession* session /* = NULL*/, uint8 tabId /*= 0*/, bool sendAllSlots /*= false*/, SlotIds *slots /*= NULL*/) const
{
    ...
    if (session)
    {
        ...
        session->SendPacket(&data);
        IC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %d",
    ...
    else /// @todo - Probably this is just sent to session + those that have sent CMSG_GUILD_BANKER_ACTIVATE
    {
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        {
            ...
            data.put<uint32>(rempos, numSlots);
            player->GetSession()->SendPacket(&data);
            IC_LOG_DEBUG("guild", "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %u"
