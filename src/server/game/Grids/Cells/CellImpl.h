...
#ifndef INFINITY_CELLIMPL_H
#define INFINITY_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Infinity::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Infinity::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Infinity::ComputeCellCoord(x + radius, y + radius).normalize();
