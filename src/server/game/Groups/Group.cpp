...
m_dungeonDifficulty(DUNGEON_DIFFICULTY_NORMAL), m_raidDifficulty(RAID_DIFFICULTY_NORMAL),
...
Group::~Group()
{
    if (m_bgGroup)
    {
        IC_LOG_DEBUG("bg.battleground", "Group::~Group: battleground group being deleted.");
        ...
        else IC_LOG_ERROR("misc", "Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
...
void Group::SendLootStartRollToPlayer(uint32 countDown, uint32 mapId, Player* p, Roll const& r)
{
    if (!p || !p->GetSession())
        return;
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //IC_LOG_DEBUG("misc", "Group::GroupLoot: missing item prototype for item with id: %d", i->itemid);
            continue;
        }
            ...
            if (r->totalPlayersRolling > 0)
            {
                ...
                for (Roll::PlayerVote::const_iterator itr = r->playerVote.begin(); itr != r->playerVote.end(); ++itr)
                {
                    ...
                    if (itr->second == PASS)
                        ...
                        SendLootStartRollToPlayer(60000, lootedObject->GetMapId(), p, *r);
                }
...
{
    IC_LOG_DEBUG("network", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST)");
...
bool Group::InCombatToInstance(uint32 instanceId)
{
    ...
    {
        Player* player = itr->GetSource();
        if (player && !player->getAttackers().empty() && player->GetInstanceId() == instanceId)
...
InstanceGroupBind* Group::BindToInstance(InstanceSave* save, bool permanent, bool load)
{
    ...
    bind.perm = permanent;
    if (!load)
        IC_LOG_DEBUG("maps", "Group::BindToInstance: Group (guid: %u, storage id: %u) is now bound to map %d, instance %d, difficulty %d",
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (pp && pp->IsInWorld())
        {
            ...
            IC_LOG_DEBUG("misc", "-- Forced group value update for '%s'", pp->GetName().c_str());
        }
    }
}
