...
public:
    ...
    static bool HandleMmapTestArea(ChatHandler* handler, char const* /*args*/)
    {
        ...
        CellCoord pair(Infinity::ComputeCellCoord(object->GetPositionX(), object->GetPositionY()));
        Cell cell(pair);
        cell.SetNoCreate();

        std::list<Creature*> creatureList;

        Infinity::AnyUnitInObjectRangeCheck go_check(object, radius);
        Infinity::CreatureListSearcher<Infinity::AnyUnitInObjectRangeCheck> go_search(object, creatureList, go_check);
        TypeContainerVisitor<Infinity::CreatureListSearcher<Infinity::AnyUnitInObjectRangeCheck>, GridTypeMapContainer> go_visit(go_search);
