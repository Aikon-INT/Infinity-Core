...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    {
        SendNotification(GetInfinityString(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    if (!receiverGuid)
    {
        IC_LOG_INFO("network", "Player %u is sending mail to %s (GUID: not existed!) with subject %s "
    ...
    IC_LOG_INFO("network", "Player %u is sending mail to %s (GUID: %u) with subject %s and body %s "
    ...
    {
        SendNotification(GetInfinityString(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr->GetPlayerNameByGUID(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetInfinityStringForDBCLocale(LANG_UNKNOWN);
                }
...
void WorldSession::HandleMailCreateTextItem(WorldPacket& recvData)
{
    ...
    IC_LOG_INFO("network", "HandleMailCreateTextItem mailid=%u", mailId);
...
void WorldSession::HandleMsgQueryNextMailtime(WorldPacket & /*recvData*/)
{
