...
void CharacterDatabaseCleaner::CleanDatabase()
{
    ...
    IC_LOG_INFO("misc", "Cleaning character database...");
    ...
    IC_LOG_INFO("server.loading", ">> Cleaned character database in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDatabaseCleaner::CheckUnique(const char* column, const char* table, bool (*check)(uint32))
{
    ...
    if (!result)
    {
        IC_LOG_INFO("misc", "Table %s is empty.", table);
        return;
    }
