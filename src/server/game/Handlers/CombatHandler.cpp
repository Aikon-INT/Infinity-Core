...
void WorldSession::HandleAttackSwingOpcode(WorldPacket& recvData)
{
    ...
    IC_LOG_DEBUG("network", "WORLD: Recvd CMSG_ATTACKSWING Message guidlow:%u guidhigh:%u", GUID_LOPART(guid), GUID_HIPART(guid));
...
void WorldSession::HandleSetSheathedOpcode(WorldPacket& recvData)
{
    ...
    //IC_LOG_DEBUG("network", "WORLD: Recvd CMSG_SETSHEATHED Message guidlow:%u value1:%u", GetPlayer()->GetGUIDLow(), sheathed);

    if (sheathed >= MAX_SHEATH_STATE)
    {
        IC_LOG_ERROR("network", "Unknown sheath state %u ??", sheathed);
        return;
    }
