...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    {
        IC_LOG_ERROR("network", "CHAT: Wrong message type received: %u", type);
        recvData.rfinish();
        return;
    }
    ...
    //IC_LOG_DEBUG("CHAT: packet received. type %u, lang %u", type, lang);
    ...
    if (lang == LANG_ADDON)
    {
        ...
        switch (type)
        {
            ...
            case CHAT_MSG_WHISPER:
                ...
                break;
            default:
                IC_LOG_ERROR("network", "Player %s (GUID: %u) sent a chatmessage with an invalid language/message type combination",
    ...
    else
    {
        ...
        if (!sender->CanSpeak())
        {
            ...
            SendNotification(GetInfinityString(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
    ...
    {
        SendNotification(GetInfinityString(LANG_GM_SILENCE), sender->GetName().c_str());
        recvData.rfinish();
        return;
    }
    ...
    if (!ignoreChecks)
    {
        ...
        if (lang != LANG_ADDON)
        {
            ...
            {
                IC_LOG_ERROR("network", "Player %s (GUID: %u) sent a chatmessage with an invalid link: %s", GetPlayer()->GetName().c_str(),
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetInfinityString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
                SendNotification(GetInfinityString(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
/* fix me.
...
*/
            ...
            {
                SendNotification(GetInfinityString(LANG_GM_SILENCE), GetPlayer()->GetName().c_str());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (!HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHAT_CHANNEL_REQ))
            {
                ...
                {
                    SendNotification(GetInfinityString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            if (!_player->IsInCombat())
            {
                ...
                {
                    _player->autoReplyMsg = msg.empty() ? GetInfinityString(LANG_PLAYER_AFK_DEFAULT) : msg;
        ...
        case CHAT_MSG_DND:
        {
            ...
            {
                _player->autoReplyMsg = msg.empty() ? GetInfinityString(LANG_PLAYER_DND_DEFAULT) : msg;
        ...
        default:
            IC_LOG_ERROR("network", "CHAT: unknown message type %u, lang: %u", type, lang);
            break;
    }
}
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
    ...
    CellCoord p = Infinity::ComputeCellCoord(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

    Infinity::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
    Infinity::LocalizedPacketDo<Infinity::EmoteChatBuilder > emote_do(emote_builder);
    Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
...
void WorldSession::HandleChatIgnoredOpcode(WorldPacket& recvData)
{
    uint64 iguid;
    uint8 unk;
    //IC_LOG_DEBUG("network", "WORLD: Received CMSG_CHAT_IGNORED");
...
{
    IC_LOG_DEBUG("network", "Opcode %u", recvPacket.GetOpcode());
}
