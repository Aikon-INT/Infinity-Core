...
void hyjalAI::Reset()
{
    ...
    } else IC_LOG_ERROR("scripts", ERROR_INST_DATA);
...
void hyjalAI::SummonNextWave(const Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if (!instance)
    {
        IC_LOG_ERROR("scripts", ERROR_INST_DATA);
        return;
    }
    ...
    if (!wave[Count].IsBoss)
    {
        ...
        {
            NextWaveTimer = 15000;
            IC_LOG_DEBUG("scripts", "HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
...
{
    CellCoord pair(Infinity::ComputeCellCoord(x, y));
    Cell cell(pair);
    cell.SetNoCreate();

    // First get all creatures.
    std::list<Creature*> creatures;
    Infinity::AllFriendlyCreaturesInGrid creature_check(me);
    Infinity::CreatureListSearcher<Infinity::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <Infinity::CreatureListSearcher<Infinity::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    Infinity::WorldObjectWorker<Infinity::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <Infinity::CreatureListSearcher<Infinity::AllFriendlyCreaturesInGrid>,
