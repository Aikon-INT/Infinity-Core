...
public:
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        ...
        IC_LOG_DEBUG("misc", handler->GetInfinityString(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (moneyToAdd < 0)
        {
            ...
            IC_LOG_DEBUG("misc", handler->GetInfinityString(LANG_CURRENT_MONEY), targetMoney, moneyToAdd, newmoney);
            if (newmoney <= 0)
            {
        ...
        IC_LOG_DEBUG("misc", handler->GetInfinityString(LANG_NEW_MONEY), targetMoney, moneyToAdd, target->GetMoney());
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetInfinityString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
