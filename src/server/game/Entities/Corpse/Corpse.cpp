...
bool Corpse::Create(uint32 guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        IC_LOG_ERROR("entities.player", "Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    _gridCoord = Infinity::ComputeGridCoord(GetPositionX(), GetPositionY());
...
void Corpse::DeleteBonesFromWorld()
{
    ...
    if (!corpse)
    {
        IC_LOG_ERROR("entities.player", "Bones %u not found in world.", GetGUIDLow());
        return;
    }
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field* fields)
{
    ...
    // SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, guildId, flags, dynFlags, time, corpseType, instanceId, corpseGuid, guid FROM corpse WHERE corpseType <> 0
    ...
    if (!IsPositionValid())
    {
        IC_LOG_ERROR("entities.player", "Corpse (guid: %u, owner: %u) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
