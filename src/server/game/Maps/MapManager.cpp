...
bool MapManager::CanPlayerEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    if (entry->IsRaid())
    {
        ...
        if ((!group || !group->isRaidGroup()) && !sWorld->getBoolConfig(CONFIG_INSTANCE_IGNORE_RAID))
        {
            ...
            player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetInfinityString(LANG_INSTANCE_RAID_GROUP_ONLY), mapName);
            IC_LOG_DEBUG("maps", "MAP: Player '%s' must be in a raid group to enter instance '%s'", player->GetName().c_str(), mapName);
            return false;
        }
    }
...
/*
            if (!corpseMap)
            {
                ...
                IC_LOG_DEBUG("maps", "MAP: Player '%s' does not have a corpse in instance '%s' and cannot enter.", player->GetName().c_str(), mapName);
                return false;
            }
*/
            IC_LOG_DEBUG("maps", "MAP: Player '%s' has corpse in instance '%s' and can enter.", player->GetName().c_str(), mapName);
            player->ResurrectPlayer(0.5f, false);
            player->SpawnCorpseBones();
        }
        else
            IC_LOG_DEBUG("maps", "Map::CanPlayerEnter - player '%s' is dead but does not have a corpse!", player->GetName().c_str());
    }
    ...
    if (entry->IsDungeon() && (!player->GetGroup()))
    {
...
uint32 MapManager::GenerateInstanceId()
{
    ...
    {
        IC_LOG_ERROR("maps", "Instance ID overflow!! Can't continue, shutting down server. ");
