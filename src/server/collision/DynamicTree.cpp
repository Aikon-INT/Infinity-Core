...
struct DynamicTreeIntersectionCallback_WithLogger
{
    ...
    {
        IC_LOG_DEBUG("maps", "Dynamic Intersection log");
    }
    bool operator()(const G3D::Ray& r, const GameObjectModel& obj, float& distance)
    {
        IC_LOG_DEBUG("maps", "testing intersection with %s", obj.name.c_str());
        bool hit = obj.intersectRay(r, distance, true);
        if (hit)
        {
            did_hit = true;
            IC_LOG_DEBUG("maps", "result: intersects");
        }
        return hit;
    }
    bool didHit() const { return did_hit;}
...
bool DynamicMapTree::getIntersectionTime(const G3D::Ray& ray,
...
bool DynamicMapTree::getObjectHitPos(const G3D::Vector3& startPos,
