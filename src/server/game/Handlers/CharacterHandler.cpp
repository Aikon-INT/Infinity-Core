...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            IC_LOG_INFO("network", "Loading char guid %u from account %u.", guidlow, GetAccountId());
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        ...
        SendPacket(&data);
        IC_LOG_ERROR("network", "Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", class_, GetAccountId());
        return;
    }
    ...
    if (!raceEntry)
    {
        data << uint8(CHAR_CREATE_FAILED);
        SendPacket(&data);
        IC_LOG_ERROR("network", "Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", race_, GetAccountId());
        return;
    }
    ...
    if (raceEntry->expansion > Expansion())
    {
        ...
        IC_LOG_ERROR("network", "Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", Expansion(), GetAccountId(), raceEntry->expansion, race_);
        SendPacket(&data);
        return;
    }
    ...
    /*if (classEntry->expansion > Expansion())
    {
        ...
        IC_LOG_ERROR("network", "Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", Expansion(), GetAccountId(), classEntry->expansion, class_);
        SendPacket(&data);
        return;
    }*/
    ...
    if (!normalizePlayerName(name))
    {
        ...
        SendPacket(&data);
        IC_LOG_ERROR("network", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
        return;
    }
    ...
    /*switch (_charCreateCallback.GetStage())
    {
        ...
        case 3:
        {
            ...
            if (createInfo->Data.rpos() < createInfo->Data.wpos())
            {
                uint8 unk;
                createInfo->Data >> unk;
                IC_LOG_DEBUG("network", "Character creation %s (account %u) has unhandled tail data: [%u]", createInfo->Name.c_str(), GetAccountId(), unk);
            }
            ...
            IC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    std::string IP_str = GetRemoteAddress();
    IC_LOG_INFO("entities.player.character", "Account: %d, IP: %s deleted character: %s, GUID: %u, Level: %u", accountId, IP_str.c_str(), name.c_str(), GUID_LOPART(guid), level);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    {
        IC_LOG_ERROR("network", "Player tryes to login again, AccountId = %d", GetAccountId());
        return;
    }
    ...
    IC_LOG_DEBUG("network", "WORLD: Recvd Player Logon Message");

    recvData >> playerGuid;

    if (!IsLegitCharacterForAccount(GUID_LOPART(playerGuid)))
    {
        IC_LOG_ERROR("network", "Account (%u) can't login with that character (%u).", GetAccountId(), GUID_LOPART(playerGuid));
        KickPlayer();
        return;
    }
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    // Send MOTD
    {
        ...
        SendPacket(&data);
        IC_LOG_DEBUG("network", "WORLD: Sent motd (SMSG_MOTD)");

        // send server info
        if (sWorld->getIntConfig(CONFIG_ENABLE_SINFO_LOGIN) == 1)
            chH.PSendSysMessage(_FULLVERSION);

        IC_LOG_DEBUG("network", "WORLD: Sent server info");
    }
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            IC_LOG_ERROR("network", "Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", pCurrChar->GetName().c_str(), pCurrChar->GetGUIDLow(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
    ...
    //IC_LOG_DEBUG("Player %s added to Map.", pCurrChar->GetName().c_str());
    ...
    std::string IP_str = GetRemoteAddress();
    IC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
...
{
    IC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_ATWAR");
...
{
    IC_LOG_ERROR("network", "WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
void WorldSession::HandleSetWatchedFactionIndexOpcode(WorldPacket& recvData)
{
    IC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_WATCHED_FACTION");
    uint32 fact;
    recvData >> fact;
    GetPlayer()->SetUInt32Value(PLAYER_FIELD_WATCHED_FACTION_INDEX, fact);
}

void WorldSession::HandleSetWatchedFactionInactiveOpcode(WorldPacket& recvData)
{
    IC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_INACTIVE");
    uint32 replistid;
    uint8 inactive;
...
void WorldSession::HandleToggleHelmOpcode(WorldPacket& recvData)
{
    IC_LOG_DEBUG("network", "CMSG_SHOWING_HELM for %s", _player->GetName().c_str());
    recvData.read_skip<uint8>(); // unknown, bool?
    _player->ToggleFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_HELM);
}

void WorldSession::HandleToggleCloakOpcode(WorldPacket& recvData)
{
    IC_LOG_DEBUG("network", "CMSG_SHOWING_CLOAK for %s", _player->GetName().c_str());
    recvData.read_skip<uint8>(); // unknown, bool?
    _player->ToggleFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_CLOAK);
}

void WorldSession::HandleChangePlayerNameOpcode(WorldPacket& recvData)
{
    uint64 guid;
    std::string newName;
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string const& newName)
{
    ...
    IC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Character:[%s] (guid:%u) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow, newName.c_str());
...
*/
void WorldSession::HandleDeclinedPlayerNameOpcode(WorldPacket& recvData)
{
    uint64 guid;
