        ...
        //        DoZoneInCombat();
            ...
            bool HatchAllEggs(uint32 action) //1: reset, 2: isHatching all
            {
...

                {
                    ...
                    Infinity::AllCreaturesOfEntryInRange check(me, NPC_EGG, 100);
                    Infinity::CreatureListSearcher<Infinity::AllCreaturesOfEntryInRange> searcher(me, templist, check);

                    TypeContainerVisitor<Infinity::CreatureListSearcher<Infinity::AllCreaturesOfEntryInRange>, GridTypeMapContainer> cSearcher(searcher);

                    cell.Visit(pair, cSearcher, *me->GetMap(), *me, me->GetGridActivationRange());
                }

                //IC_LOG_ERROR("scripts", "Eggs %d at middle", templist.size());
                if (templist.empty())
                    return false;
            ...
            void Boom()
            {
...

                {
                    ...
                    Infinity::AllCreaturesOfEntryInRange check(me, NPC_FIRE_BOMB, 100);
                    Infinity::CreatureListSearcher<Infinity::AllCreaturesOfEntryInRange> searcher(me, templist, check);
        ...
        struct npc_janalai_hatcherAI : public ScriptedAI
        {
            ...
            bool HatchEggs(uint32 num)
            {
...

                {
                    ...
                    Infinity::AllCreaturesOfEntryInRange check(me, 23817, 50);
                    Infinity::CreatureListSearcher<Infinity::AllCreaturesOfEntryInRange> searcher(me, templist, check);
                ...
                //IC_LOG_ERROR("scripts", "Eggs %d at %d", templist.size(), side);
