...
{
    IC_LOG_DEBUG("warden", "Send module to client");
...
{
    IC_LOG_DEBUG("warden", "Request module");
    ...
    if (_initialized)
    {
        ...
        if (_dataSent)
        {
            ...
            if (maxClientResponseDelay > 0)
            {
                ...
                {
                    IC_LOG_WARN("warden", "%s (latency: %u, IP: %s) exceeded Warden module response delay for more than %s - disconnecting client",
...
bool Warden::IsValidCheckSum(uint32 checksum, const uint8* data, const uint16 length)
{
    ...
    {
        IC_LOG_DEBUG("warden", "CHECKSUM IS NOT VALID");
        return false;
    }
    else
    {
        IC_LOG_DEBUG("warden", "CHECKSUM IS VALID");
        return true;
    }
}
...
void WorldSession::HandleWardenDataOpcode(WorldPacket& recvData)
{
    ...
    uint8 opcode;
    recvData >> opcode;
    IC_LOG_DEBUG("warden", "Got packet, opcode %02X, size %u", opcode, uint32(recvData.size()));
    recvData.hexlike();
    ...
    switch (opcode)
    {
        ...
        case WARDEN_CMSG_MEM_CHECKS_RESULT:
            IC_LOG_DEBUG("warden", "NYI WARDEN_CMSG_MEM_CHECKS_RESULT received!");
            break;
        ...
        case WARDEN_CMSG_MODULE_FAILED:
            IC_LOG_DEBUG("warden", "NYI WARDEN_CMSG_MODULE_FAILED received!");
            break;
        default:
            IC_LOG_DEBUG("warden", "Got unknown warden opcode %02X of size %u.", opcode, uint32(recvData.size() - 1));
            break;
    }
}
