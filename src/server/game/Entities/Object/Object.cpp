...
WorldObject::~WorldObject()
{
    ...
    if (IsWorldObject() && m_currMap)
    {
        ...
        {
            IC_LOG_FATAL("misc", "Object::~Object Corpse guid=" UI64FMTD ", type=%d, entry=%u deleted but still in map!!",
...
Object::~Object()
{
    if (IsInWorld())
    {
        IC_LOG_FATAL("misc", "Object::~Object - guid=" UI64FMTD ", typeid=%d, entry=%u deleted but still in world!!", GetGUID(), GetTypeId(), GetEntry());
        if (isType(TYPEMASK_ITEM))
            IC_LOG_FATAL("misc", "Item slot %u", ((Item*)this)->GetSlot());
        ASSERT(false);
        RemoveFromWorld();
    }
    ...
    if (m_objectUpdated)
    {
        IC_LOG_FATAL("misc", "Object::~Object - guid=" UI64FMTD ", typeid=%d, entry=%u deleted but still in update list!!", GetGUID(), GetTypeId(), GetEntry());
        ASSERT(false);
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    ...
    //IC_LOG_DEBUG("BuildCreateUpdate: update-type: %u, object-type: %u got flags: %X, flags2: %X", updateType, m_objectTypeId, flags, flags2);
...
void Object::SetByteValue(uint16 index, uint8 offset, uint8 value)
{
    ...
    if (offset > 4)
    {
        IC_LOG_ERROR("misc", "Object::SetByteValue: wrong offset %u", offset);
        return;
    }
...
void Object::SetUInt16Value(uint16 index, uint8 offset, uint16 value)
{
    ...
    if (offset > 2)
    {
        IC_LOG_ERROR("misc", "Object::SetUInt16Value: wrong offset %u", offset);
        return;
    }
...
void Object::SetByteFlag(uint16 index, uint8 offset, uint8 newFlag)
{
    ...
    if (offset > 4)
    {
        IC_LOG_ERROR("misc", "Object::SetByteFlag: wrong offset %u", offset);
        return;
    }
...
void Object::RemoveByteFlag(uint16 index, uint8 offset, uint8 oldFlag)
{
    ...
    if (offset > 4)
    {
        IC_LOG_ERROR("misc", "Object::RemoveByteFlag: wrong offset %u", offset);
        return;
    }
...
{
    IC_LOG_ERROR("misc", "Attempt %s non-existed value field: %u (count: %u) for object typeid: %u type mask: %u", (set ? "set value to" : "get value from"), index, m_valuesCount, GetTypeId(), m_objectType);
...
{
    IC_LOG_INFO("misc", "MOVEMENT INFO");
    IC_LOG_INFO("misc", "guid " UI64FMTD, guid);
    IC_LOG_INFO("misc", "flags %u", flags);
    IC_LOG_INFO("misc", "flags2 %u", flags2);
    IC_LOG_INFO("misc", "time %u current time " UI64FMTD "", flags2, uint64(::time(NULL)));
    IC_LOG_INFO("misc", "position: `%s`", pos.ToString().c_str());
    if (flags & MOVEMENTFLAG_ONTRANSPORT)
    {
        IC_LOG_INFO("misc", "TRANSPORT:");
        IC_LOG_INFO("misc", "guid: " UI64FMTD, transport.guid);
        IC_LOG_INFO("misc", "position: `%s`", transport.pos.ToString().c_str());
        IC_LOG_INFO("misc", "seat: %i", transport.seat);
        IC_LOG_INFO("misc", "time: %u", transport.time);
        if (flags2 & MOVEMENTFLAG2_INTERPOLATED_MOVEMENT)
            IC_LOG_INFO("misc", "time2: %u", transport.time2);
    }

    if ((flags & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) || (flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING))
        IC_LOG_INFO("misc", "pitch: %f", pitch);

    IC_LOG_INFO("misc", "fallTime: %u", fallTime);
    if (flags & MOVEMENTFLAG_FALLING)
        IC_LOG_INFO("misc", "j_zspeed: %f j_sinAngle: %f j_cosAngle: %f j_xyspeed: %f", jump.zspeed, jump.sinAngle, jump.cosAngle, jump.xyspeed);

    if (flags & MOVEMENTFLAG_SPLINE_ELEVATION)
        IC_LOG_INFO("misc", "splineElevation: %f", splineElevation);
}
...
void WorldObject::GetRandomPoint(const Position &pos, float distance, float &rand_x, float &rand_y, float &rand_z) const
{
    ...
    Infinity::NormalizeMapCoord(rand_x);
    Infinity::NormalizeMapCoord(rand_y);
...
{
    return Infinity::IsValidMapCoord(m_positionX, m_positionY, m_positionZ, m_orientation);
}
...
{
    CellCoord p = Infinity::ComputeCellCoord(GetPositionX(), GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

    Infinity::MonsterCustomChatBuilder say_build(this, CHAT_MSG_MONSTER_SAY, text, language, target);
    Infinity::LocalizedPacketDo<Infinity::MonsterCustomChatBuilder> say_do(say_build);
    Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterCustomChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
    TypeContainerVisitor<Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterCustomChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void WorldObject::MonsterSay(int32 textId, uint32 language, WorldObject const* target)
{
    ...
    Infinity::MonsterChatBuilder say_build(this, CHAT_MSG_MONSTER_SAY, textId, language, target);
    Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> say_do(say_build);
    Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
    TypeContainerVisitor<Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
    cell.Visit(p, message, *GetMap(), *this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY));
}
...
void WorldObject::MonsterYell(const char* text, uint32 language, WorldObject const* target)
{
    ...
    Infinity::MonsterCustomChatBuilder say_build(this, CHAT_MSG_MONSTER_YELL, text, language, target);
    Infinity::LocalizedPacketDo<Infinity::MonsterCustomChatBuilder> say_do(say_build);
    Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterCustomChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_YELL), say_do);
    TypeContainerVisitor<Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterCustomChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void WorldObject::MonsterYell(int32 textId, uint32 language, WorldObject const* target)
{
    ...
    Infinity::MonsterChatBuilder say_build(this, CHAT_MSG_MONSTER_YELL, textId, language, target);
    Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> say_do(say_build);
    Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_YELL), say_do);
    TypeContainerVisitor<Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
    cell.Visit(p, message, *GetMap(), *this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_YELL));
}
...
void WorldObject::MonsterTextEmote(int32 textId, WorldObject const* target, bool IsBossEmote)
{
    ...
    Infinity::MonsterChatBuilder say_build(this, IsBossEmote ? CHAT_MSG_RAID_BOSS_EMOTE : CHAT_MSG_MONSTER_EMOTE, textId, LANG_UNIVERSAL, target);
    Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> say_do(say_build);
    Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), say_do);
    TypeContainerVisitor<Infinity::PlayerDistWorker<Infinity::LocalizedPacketDo<Infinity::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
void WorldObject::MonsterWhisper(int32 textId, Player const* target, bool IsBossWhisper)
{
    ...
    LocaleConstant loc_idx = target->GetSession()->GetSessionDbLocaleIndex();
    char const* text = sObjectMgr->GetInfinityString(textId, loc_idx);
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, IsBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER : CHAT_MSG_MONSTER_WHISPER, LANG_UNIVERSAL, this, target, text, 0, "", loc_idx);
...
void WorldObject::SetMap(Map* map)
{
    ...
    if (m_currMap)
    {
        IC_LOG_FATAL("misc", "WorldObject::SetMap: obj %u new map %u %u, old map %u %u", (uint32)GetTypeId(), map->GetId(), map->GetInstanceId(), m_currMap->GetId(), m_currMap->GetInstanceId());
        ASSERT(false);
    }
    m_currMap = map;
    m_mapId = map->GetId();
...
void WorldObject::AddObjectToRemoveList()
{
    ...
    if (!map)
    {
        IC_LOG_ERROR("misc", "Object (TypeId: %u Entry: %u GUID: %u) at attempt add to move list not have valid map (Id: %u).", GetTypeId(), GetEntry(), GetGUIDLow(), GetMapId());
        return;
    }
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position &pos, TempSummonType spwtype, uint32 duration) const
{
    if (Map* map = FindMap())
    {
        if (TempSummon* summon = map->SummonCreature(entry, pos, NULL, duration))
        {
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime)
{
    ...
    if (!goinfo)
    {
        IC_LOG_ERROR("sql.sql", "Gameobject template %u not found in database!", entry);
        return NULL;
    }
...
Creature* WorldObject::FindNearestCreature(uint32 entry, float range, bool alive) const
{
    ...
    Infinity::NearestCreatureEntryWithLiveStateInObjectRangeCheck checker(*this, entry, alive, range);
    Infinity::CreatureLastSearcher<Infinity::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(this, creature, checker);
...
{
    GameObject* go = NULL;
    Infinity::NearestGameObjectEntryInObjectRangeCheck checker(*this, entry, range);
    Infinity::GameObjectLastSearcher<Infinity::NearestGameObjectEntryInObjectRangeCheck> searcher(this, go, checker);
...
{
    GameObject* go = NULL;
    Infinity::NearestGameObjectTypeInObjectRangeCheck checker(*this, type, range);
    Infinity::GameObjectLastSearcher<Infinity::NearestGameObjectTypeInObjectRangeCheck> searcher(this, go, checker);
    VisitNearbyGridObject(range, searcher);
    return go;
}
...
{
    CellCoord pair(Infinity::ComputeCellCoord(this->GetPositionX(), this->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    Infinity::AllGameObjectsWithEntryInRange check(this, entry, maxSearchRange);
    Infinity::GameObjectListSearcher<Infinity::AllGameObjectsWithEntryInRange> searcher(this, gameobjectList, check);
    TypeContainerVisitor<Infinity::GameObjectListSearcher<Infinity::AllGameObjectsWithEntryInRange>, GridTypeMapContainer> visitor(searcher);
...
void WorldObject::GetCreatureListWithEntryInGrid(std::list<Creature*>& creatureList, uint32 entry, float maxSearchRange) const
{
    ...
    Infinity::AllCreaturesOfEntryInRange check(this, entry, maxSearchRange);
    Infinity::CreatureListSearcher<Infinity::AllCreaturesOfEntryInRange> searcher(this, creatureList, check);
    TypeContainerVisitor<Infinity::CreatureListSearcher<Infinity::AllCreaturesOfEntryInRange>, GridTypeMapContainer> visitor(searcher);
...
void WorldObject::GetNearPoint2D(float &x, float &y, float distance2d, float absAngle) const
{
    ...
    Infinity::NormalizeMapCoord(x);
    Infinity::NormalizeMapCoord(y);
}
...
void WorldObject::GetNearPoint(WorldObject const* /*searcher*/, float &x, float &y, float &z, float searcher_size, float distance2d, float absAngle) const
{
    ...
    // adding used positions around object
    {
        ...
        Infinity::NearUsedPosDo u_do(*this, searcher, absAngle, selector);
        Infinity::WorldObjectWorker<Infinity::NearUsedPosDo> worker(this, u_do);

        TypeContainerVisitor<Infinity::WorldObjectWorker<Infinity::NearUsedPosDo>, GridTypeMapContainer  > grid_obj_worker(worker);
        TypeContainerVisitor<Infinity::WorldObjectWorker<Infinity::NearUsedPosDo>, WorldTypeMapContainer > world_obj_worker(worker);
...
void WorldObject::MovePosition(Position &pos, float dist, float angle)
{
    ...
    if (!Infinity::IsValidMapCoord(destx, desty, pos.m_positionZ))
    {
        IC_LOG_FATAL("misc", "WorldObject::MovePosition: Object (TypeId: %u Entry: %u GUID: %u) has invalid coordinates X: %f and Y: %f were passed!",
    ...
    Infinity::NormalizeMapCoord(pos.m_positionX);
    Infinity::NormalizeMapCoord(pos.m_positionY);
...
void WorldObject::MovePositionToFirstCollision(Position &pos, float dist, float angle)
{
    ...
    // Prevent invalid coordinates here, position is unchanged
    if (!Infinity::IsValidMapCoord(destx, desty))
    {
        IC_LOG_FATAL("misc", "WorldObject::MovePositionToFirstCollision invalid coordinates X: %f and Y: %f were passed!", destx, desty);
        return;
    }
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    Infinity::AnyPlayerInObjectRangeCheck check(this, GetVisibilityRange(), false);
    Infinity::PlayerListSearcher<Infinity::AnyPlayerInObjectRangeCheck> searcher(this, targets, check);
...
void WorldObject::UpdateObjectVisibility(bool /*forced*/)
{
    ...
    Infinity::VisibleChangesNotifier notifier(*this);
    VisitNearbyWorldObject(GetVisibilityRange(), notifier);
}
