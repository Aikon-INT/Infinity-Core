...
void RARunnable::run()
{
    ...
    {
        IC_LOG_ERROR("server.worldserver", "Infinity RA can not bind to port %d on %s", raPort, stringIp.c_str());
        return;
    }

    IC_LOG_INFO("server.worldserver", "Starting Infinity RA on port %d on %s", raPort, stringIp.c_str());
    ...
    IC_LOG_DEBUG("server.worldserver", "Infinity RA thread exiting");
}
