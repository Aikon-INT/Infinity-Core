...
{
    IC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_BUY");
    ...
    IC_LOG_DEBUG("network", "Petitioner with GUID %u tried sell petition: name %s", GUID_LOPART(guidNPC), name.c_str());

    // prevent cheating
    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);
    if (!creature)
    {
        IC_LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - Unit (GUID: %u) not found or you can't interact with him.", GUID_LOPART(guidNPC));
        return;
    }
    ...
    else
    {
        ...
        switch (clientIndex)                                 // arenaSlot+1 as received from client (1 from 3 case)
        {
            ...
            case 3:
                ...
                break;
            default:
                IC_LOG_DEBUG("network", "unknown selection at buy arena petition: %u", clientIndex);
                return;
        }
    ...
    IC_LOG_DEBUG("network", "Invalid petition GUIDs: %s", ssInvalidPetitionGUIDs.str().c_str());
...
{
    IC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SHOW_SIGNATURES");
    ...
    if (!result)
    {
        IC_LOG_DEBUG("entities.player.items", "Petition %u is not found for player %u %s", GUID_LOPART(petitionguid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName().c_str());
        return;
    }
    Field* fields = result->Fetch();
    ...
    IC_LOG_DEBUG("network", "CMSG_PETITION_SHOW_SIGNATURES petition entry: '%u'", petitionGuidLow);
...
{
    IC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_QUERY");   // ok

    uint32 guildguid;
    uint64 petitionguid;
    recvData >> guildguid;                                 // in Infinity always same as GUID_LOPART(petitionguid)
    recvData >> petitionguid;                              // petition guid
    IC_LOG_DEBUG("network", "CMSG_PETITION_QUERY Petition GUID %u Guild GUID %u", GUID_LOPART(petitionguid), guildguid);
...
void WorldSession::SendPetitionQueryOpcode(uint64 petitionguid)
{
    ...
    {
        IC_LOG_DEBUG("network", "CMSG_PETITION_QUERY failed for petition (GUID: %u)", GUID_LOPART(petitionguid));
        return;
    }

    WorldPacket data(SMSG_PETITION_QUERY_RESPONSE, (4+8+name.size()+1+1+4*12+2+10));
    data << uint32(GUID_LOPART(petitionguid));              // guild/team guid (in Infinity always same as GUID_LOPART(petition guid)
...
{
    IC_LOG_DEBUG("network", "Received opcode MSG_PETITION_RENAME");   // ok
    ...
    IC_LOG_DEBUG("network", "Petition (GUID: %u) renamed to '%s'", GUID_LOPART(petitionGuid), newName.c_str());
...
{
    IC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SIGN");    // ok
    ...
    if (!result)
    {
        IC_LOG_ERROR("network", "Petition %u is not found for player %u %s", GUID_LOPART(petitionGuid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName().c_str());
        return;
    }
    ...
    IC_LOG_DEBUG("network", "PETITION SIGN: GUID %u by player: %s (GUID: %u Account: %u)", GUID_LOPART(petitionGuid), _player->GetName().c_str(), playerGuid, GetAccountId());
...
{
    IC_LOG_DEBUG("network", "Received opcode MSG_PETITION_DECLINE");  // ok

    uint64 petitionguid;
    uint64 ownerguid;
    recvData >> petitionguid;                              // petition guid
    IC_LOG_DEBUG("network", "Petition %u declined by %u", GUID_LOPART(petitionguid), _player->GetGUIDLow());
...
{
    IC_LOG_DEBUG("network", "Received opcode CMSG_OFFER_PETITION");   // ok
    ...
    IC_LOG_DEBUG("network", "OFFER PETITION: type %u, GUID1 %u, to player id: %u", type, GUID_LOPART(petitionguid), GUID_LOPART(plguid));
...
{
    IC_LOG_DEBUG("network", "Received opcode CMSG_TURN_IN_PETITION");
    ...
    IC_LOG_DEBUG("network", "Petition %u turned in by %u", GUID_LOPART(petitionGuid), _player->GetGUIDLow());
    ...
    {
        IC_LOG_ERROR("network", "Player %s (guid: %u) tried to turn in petition (guid: %u) that is not present in the database", _player->GetName().c_str(), _player->GetGUIDLow(), GUID_LOPART(petitionGuid));
        return;
    }
    ...
    else
    {
        ...
        IC_LOG_DEBUG("network", "PetitonsHandler: Arena team (guid: %u) added to ObjectMgr", arenaTeam->GetId());
        ...
        for (uint8 i = 0; i < signatures; ++i)
        {
            ...
            IC_LOG_DEBUG("network", "PetitionsHandler: Adding arena team (guid: %u) member %u", arenaTeam->GetId(), memberGUID);
    ...
    // created
    IC_LOG_DEBUG("network", "TURN IN PETITION GUID %u", GUID_LOPART(petitionGuid));
...
{
    IC_LOG_DEBUG("network", "Received CMSG_PETITION_SHOWLIST");
...
void WorldSession::SendPetitionShowList(uint64 guid)
{
    ...
    if (!creature)
    {
        IC_LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    SendPacket(&data);
    IC_LOG_DEBUG("network", "Sent SMSG_PETITION_SHOWLIST");
}
