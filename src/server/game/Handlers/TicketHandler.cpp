...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recvData)
{
    ...
    {
        SendNotification(GetInfinityString(LANG_TICKET_REQ), sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ));
        return;
    }
    ...
    if (!ticket || ticket->IsClosed())
    {
        ...
        if (count && decompressedSize && decompressedSize < 0xFFFF)
        {
            ...
            {
                IC_LOG_ERROR("network", "CMSG_GMTICKET_CREATE possibly corrupt. Uncompression failed.");
                recvData.rfinish();
                delete ticket;
                return;
            }
...
void WorldSession::HandleGMTicketUpdateTextOpcode(WorldPacket& recvData)
{
    std::string message;
    recvData >> message;
