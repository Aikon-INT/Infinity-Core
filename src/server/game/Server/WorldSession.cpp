...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    else
    {
        ...
        IC_LOG_INFO("misc", "Send all time packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f time: %u", sendPacketCount, sendPacketBytes, float(sendPacketCount)/fullTime, float(sendPacketBytes)/fullTime, uint32(fullTime));
        IC_LOG_INFO("misc", "Send last min packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f", sendLastPacketCount, sendLastPacketBytes, float(sendLastPacketCount)/minTime, float(sendLastPacketBytes)/minTime);
...
{
    IC_LOG_ERROR("network.opcode", "Received unexpected opcode %s Status: %s Reason: %s from %s",
...
void WorldSession::LogUnprocessedTail(WorldPacket* packet)
{
    ...
    IC_LOG_TRACE("network.opcode", "Unprocessed tail data (read stop at %u from %u) Opcode %s from %s",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
            ...
            _recvQueue.next(packet, updater))
    {
        ...
        {
            IC_LOG_ERROR("network.opcode", "Received non-existed opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode()).c_str()
                            , GetPlayerInfo().c_str());
        ...
        else
        {
            ...
            try
            {
                switch (opHandle.status)
                {
                    case STATUS_LOGGEDIN:
                        if (!_player)
                        {
                            ...
                            if (!m_playerRecentlyLogout)
                            {
                                ...
                                QueuePacket(packet);
                                //! Log
                                IC_LOG_DEBUG("network", "Re-enqueueing packet with opcode %s with with status STATUS_LOGGEDIN. "
                    ...
                    case STATUS_AUTHED:
                        ...
                        break;
                    case STATUS_NEVER:
                        IC_LOG_ERROR("network.opcode", "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode()).c_str()
                            , GetPlayerInfo().c_str());
                        break;
                }
            }
            ...
            {
                IC_LOG_ERROR("misc", "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i. Skipped packet.",
...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player)
    {
        ...
        IC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Logout Character:[%s] (GUID: %u) Level: %d",
        ...
        SendPacket(&data);
        IC_LOG_DEBUG("network", "SESSION: Sent SMSG_LOGOUT_COMPLETE Message");
...
{
    char const* format = GetInfinityString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetInfinityString(int32 entry) const
{
    return sObjectMgr->GetInfinityString(entry, GetSessionDbLocaleIndex());
}

void WorldSession::Handle_NULL(WorldPacket& recvPacket)
{
    IC_LOG_ERROR("network.opcode", "Received unhandled opcode %s from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode()).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    IC_LOG_ERROR("network.opcode", "Received opcode %s that must be processed in WorldSocket::OnRead from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode()).c_str(), GetPlayerInfo().c_str());
}

void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    IC_LOG_ERROR("network.opcode", "Received server-side opcode %s from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode()).c_str(), GetPlayerInfo().c_str());
}
/*
void WorldSession::Handle_Deprecated(WorldPacket& recvPacket)
{
    IC_LOG_ERROR("network.opcode", "Received deprecated opcode %s from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode()).c_str(), GetPlayerInfo().c_str());
}
*/
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            IC_LOG_ERROR("misc", "Table `%s` have invalid account data type (%u), ignore.",
        ...
        if ((mask & (1 << type)) == 0)
        {
            IC_LOG_ERROR("misc", "Table `%s` have non appropriate for table  account data type (%u), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadMovementInfo(WorldPacket &data, MovementInfo* mi)
{
    ...
    { \
        if (check) \
        { \
            IC_LOG_DEBUG("entities.unit", "WorldSession::ReadMovementInfo: Violation of MovementFlags found (%s). " \
...
void WorldSession::ReadAddonsInfo(WorldPacket &data)
{
    ...
    if (size > 0xFFFFF)
    {
        IC_LOG_ERROR("misc", "WorldSession::ReadAddonsInfo addon info too big, size %u", size);
        return;
    }
    ...
    if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos, data.size() - pos) == Z_OK)
    {
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            IC_LOG_INFO("misc", "ADDON: Name: %s, Enabled: 0x%x, CRC: 0x%x, Unknown2: 0x%x", addonName.c_str(), enabled, crc, unk1);
            ...
            if (savedAddon)
            {
                if (addon.CRC != savedAddon->CRC)
                    IC_LOG_INFO("misc", "ADDON: %s was known, but didn't match known CRC (0x%x)!", addon.Name.c_str(), savedAddon->CRC);
                else
                    IC_LOG_INFO("misc", "ADDON: %s was known, CRC is correct (0x%x)", addon.Name.c_str(), savedAddon->CRC);
            }
            else
            {
                AddonMgr::SaveAddon(addon);

                IC_LOG_INFO("misc", "ADDON: %s (0x%x) was not known, saving...", addon.Name.c_str(), addon.CRC);
            }
        ...
        IC_LOG_DEBUG("network", "ADDON: CurrentTime: %u", currentTime);
    }
    else
        IC_LOG_ERROR("misc", "Addon packet uncompress error!");
}
...
void WorldSession::SendAddonsInfo()
{
    ...
    for (AddonsList::iterator itr = m_addonsList.begin(); itr != m_addonsList.end(); ++itr)
    {
        ...
        if (crcpub)
        {
            ...
            {
                IC_LOG_INFO("misc", "ADDON: CRC (0x%x) for addon %s is wrong (does not match expected 0x%x), sending pubkey",
...
void WorldSession::LoadPermissions()
{
    ...
    IC_LOG_DEBUG("rbac", "WorldSession::LoadPermissions [AccountId: %u, Name: %s, realmId: %d, secLevel: %u]",
...
bool WorldSession::HasPermission(uint32 permission)
{
    ...
    IC_LOG_DEBUG("rbac", "WorldSession::HasPermission [AccountId: %u, Name: %s, realmId: %d]",
...
{
    IC_LOG_DEBUG("rbac", "WorldSession::Invalidaterbac::RBACData [AccountId: %u, Name: %s, realmId: %d]",
                   _RBACData->GetId(), _RBACData->GetName().c_str(), realmID);
    delete _RBACData;
    _RBACData = NULL;
}
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket& p) const
{
    ...
    IC_LOG_INFO("network", "AntiDOS: Account %u, IP: %s, sent unacceptable packet (opc: %u, size: %u)",
    ...
    switch (_policy)
    {
        case POLICY_LOG:
            return true;
        case POLICY_KICK:
            IC_LOG_INFO("network", "AntiDOS: Player kicked!");
            return false;
        case POLICY_BAN:
        {
            ...
            IC_LOG_INFO("network", "AntiDOS: Player automatically banned for %u seconds.", duration);
