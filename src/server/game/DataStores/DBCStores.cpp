...
{
    uint32 accepted_versions[] = EXPECTED_INFINITYCORE_CLIENT_BUILD;
...
std::string AcceptableClientBuildsListStr()
{
    ...
    int accepted_versions[] = EXPECTED_INFINITYCORE_CLIENT_BUILD;
    for (int i = 0; accepted_versions[i]; ++i)
...
{
    IC_LOG_ERROR("misc", "Size of '%s' setted by format string (%u) not equal size of C++ structure (%u).",filename.c_str(),fsize,rsize);
...
LoadDBC(availableDbcLocales, bad_dbc_files, sSpellCategoryStore,          dbcPath, "SpellCategory.dbc");
    ...
    for (uint32 i = 0; i < sSpellDifficultyStore.GetNumRows(); ++i)
    {
        ...
        for (int x = 0; x < TOTAL_DIFFICULTIES; ++x)
        {
            if (spellDiff->SpellID[x] <= 0 || !sSpellStore.LookupEntry(spellDiff->SpellID[x]))
            {
                if (spellDiff->SpellID[x] > 0)//don't show error if spell is <= 0, not all modes have spells and there are unknown negative values
                    IC_LOG_ERROR("sql.sql", "spelldifficulty_dbc: spell %i at field id:%u at spellid%i does not exist in SpellStore (spell.dbc), loaded as 0", spellDiff->SpellID[x], spellDiff->ID, x);
...
*/
    ...
    {
        IC_LOG_ERROR("misc", "Incorrect DataDir value in InfinityWorld.conf or ALL required *.dbc files (%d) not found by path: %sdbc",DBCFilesCount,dataPath.c_str());
        exit(1);
    }
    ...
    IC_LOG_INFO("server.loading", ">> Initialized %d data stores in %u ms", DBCFileCount, GetMSTimeDiffToNow(oldMSTime));
}
