...
class DatabaseWorkerPool
{
    public:
        ...
        _queue(new ACE_Activation_Queue())
        {
            ...
            WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION, "InfinityCore does not support MySQL versions below 5.1");
        }
        ...
        bool Open(const std::string& infoString, uint8 async_threads, uint8 synch_threads)
        {
            ...
            IC_LOG_INFO("sql.driver", "Opening DatabasePool '%s'. Asynchronous connections: %u, synchronous connections: %u.",
            ...
            for (uint8 i = 0; i < async_threads; ++i)
            {
                ...
                if (res) // only check mysql version if connection is valid
                    WPFatal(mysql_get_server_version(t->GetHandle()) >= MIN_MYSQL_SERVER_VERSION, "InfinityCore does not support MySQL versions below 5.1");
            ...
            if (res)
                IC_LOG_INFO("sql.driver", "DatabasePool '%s' opened successfully. %u total connections running.", GetDatabaseName(),
                    (_connectionCount[IDX_SYNCH] + _connectionCount[IDX_ASYNC]));
            else
                IC_LOG_ERROR("sql.driver", "DatabasePool %s NOT opened. There were errors opening the MySQL connections. Check your SQLDriverLogFile "
        ...
        void Close()
        {
            IC_LOG_INFO("sql.driver", "Closing down DatabasePool '%s'.", GetDatabaseName());
            ...
            IC_LOG_INFO("sql.driver", "Asynchronous connections on DatabasePool '%s' terminated. Proceeding with synchronous connections.",
                GetDatabaseName());
            ...
            IC_LOG_INFO("sql.driver", "All connections on DatabasePool '%s' closed.", GetDatabaseName());
        }
        ...
        */
        ...
        void CommitTransaction(SQLTransaction transaction)
        {
            ...
            {
                case 0:
                    IC_LOG_DEBUG("sql.driver", "Transaction contains 0 queries. Not executing.");
                    return;
                case 1:
                    IC_LOG_DEBUG("sql.driver", "Warning: Transaction only holds 1 query, consider removing Transaction context in code.");
                    break;
                default:
                    break;
            }
