...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (!m_QueueUpdateScheduler.empty())
    {
        ...
        for (uint8 i = 0; i < scheduled.size(); i++)
        {
            ...
            m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(diff, bgTypeId, arenaType, arenaMMRating > 0, arenaMMRating);
        }
    }
...
void BattlegroundMgr::BuildBattlegroundStatusPacket(WorldPacket* data, Battleground* bg, uint8 QueueSlot, uint8 StatusID, uint32 Time1, uint32 Time2, uint8 arenatype, uint32 arenaFaction)
{
    ...
    *data << uint8(arenatype);
    IC_LOG_DEBUG("network", "BattlegroundMgr::BuildBattlegroundStatusPacket: arenatype = %u for bg instanceID %u, TypeID %u.", arenatype, bg->GetClientInstanceID(), bg->GetTypeID());
    ...
    switch (StatusID)
    {
        ...
        case STATUS_IN_PROGRESS:                                // status_in_progress
            ...
            *data << uint8(arenaFaction == ALLIANCE ? 1 : 0);   // arenafaction (0 for horde, 1 for alliance)
            break;
        default:
            IC_LOG_ERROR("bg.battleground", "Unknown BG status!");
            break;
    }
}
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket* data, Battleground* bg)
{
    ...
    if (type)                                               // arena
    {
        ...
        for (int8 i = 1; i >= 0; --i)
        {
            ...
            *data << uint32(MatchmakerRating);              // Matchmaking Value
            IC_LOG_DEBUG("bg.battleground", "rating change: %d", rating_change);
        }
        for (int8 i = 1; i >= 0; --i)
        {
    ...
    *data << uint32(scoreCount);                            // placeholder
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        {
            IC_LOG_ERROR("bg.battleground", "Player " UI64FMTD " has scoreboard entry for battleground %u but is not in battleground!", itr->first, bg->GetTypeID(true));
            continue;
        }
        ...
        *data << uint32(score->HealingDone);             // healing done
        switch (bg->GetTypeID(true))                            // battleground specific things
        {
            ...
            case BATTLEGROUND_RV:
                *data << uint32(0);
                break;
            default:
                IC_LOG_DEBUG("network", "Unhandled MSG_PVP_LOG_DATA for BG id %u", bg->GetTypeID());
                *data << uint32(0);
                break;
        }
        // should never happen
        ...
        {
            IC_LOG_ERROR("bg.battleground", "Battleground %u scoreboard has more entries (%u) than allowed players in this bg (%u)", bg->GetTypeID(true), bg->GetPlayerScoresSize(), bg->GetMaxPlayers());
            break;
        }
    }
...
uint32 BattlegroundMgr::CreateClientVisibleInstanceId(BattlegroundTypeId bgTypeId)
{
    ...
    BattlegroundClientIdsContainer& clientIds = bgDataStore[bgTypeId];
    uint32 lastId = 0;
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId, uint8 arenaType, bool isRated)
{
    ...
    if (!bg_template)
    {
        IC_LOG_ERROR("bg.battleground", "Battleground: CreateNewBattleground - bg template not found for %u", bgTypeId);
        return NULL;
    }
    ...
    //bg->SetClientInstanceID(CreateClientVisibleInstanceId(isRandom ? BATTLEGROUND_RB : bgTypeId));
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    if (!result)
    {
        IC_LOG_ERROR("server.loading", ">> Loaded 0 battlegrounds. DB table `battleground_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!bl)
        {
            IC_LOG_ERROR("bg.battleground", "Battleground ID %u not found in BattlemasterList.dbc. Battleground not created.", bgTypeId);
            continue;
        }
        ...
        {
            IC_LOG_ERROR("sql.sql", "Table `battleground_template` for id %u has bad values for MinPlayersPerTeam (%u) and MaxPlayersPerTeam(%u)",
        ...
        {
            IC_LOG_ERROR("sql.sql", "Table `battleground_template` for id %u has bad values for LevelMin (%u) and LevelMax(%u)",
        ...
        else
        {
            ...
            {
                IC_LOG_ERROR("sql.sql", "Table `battleground_template` for id %u have non-existed WorldSafeLocs.dbc id %u in field `AllianceStartLoc`. BG not created.", data.bgTypeId, startId);
                continue;
            }
            ...
            {
                IC_LOG_ERROR("sql.sql", "Table `battleground_template` for id %u have non-existed WorldSafeLocs.dbc id %u in field `HordeStartLoc`. BG not created.", data.bgTypeId, startId);
                continue;
            }
        }
    ...
    IC_LOG_INFO("server.loading", ">> Loaded %u battlegrounds in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    IC_LOG_DEBUG("bg.battleground", "Initializing Automatic Arena Point Distribution");
    if (wstime < curtime)
    {
        m_NextAutoDistributionTime = curtime;           // reset will be called in the next update
        IC_LOG_DEBUG("bg.battleground", "Battleground: Next arena point distribution time in the past, reseting it now.");
    }
    else
        m_NextAutoDistributionTime = wstime;
    IC_LOG_DEBUG("bg.battleground", "Automatic Arena Point Distribution initialized.");
}
...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, uint64 guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    ...
    winner_kills = Infinity::Honor::hk_honor_at_level(player->getLevel(), float(winner_kills));
    loser_kills = Infinity::Honor::hk_honor_at_level(player->getLevel(), float(loser_kills));
...
void BattlegroundMgr::SendToBattleground(Player* player, uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    if (Battleground* bg = GetBattleground(instanceId, bgTypeId))
    {
        ...
        IC_LOG_DEBUG("bg.battleground", "BattlegroundMgr::SendToBattleground: Sending %s to map %u, X %f, Y %f, Z %f, O %f (bgType %u)", player->GetName().c_str(), mapid, x, y, z, O, bgTypeId);
        player->TeleportTo(mapid, x, y, z, O);
    }
    else
        IC_LOG_ERROR("bg.battleground", "BattlegroundMgr::SendToBattleground: Instance %u (bgType %u) not found while trying to teleport player %s", instanceId, bgTypeId, player->GetName().c_str());
}
...
void BattlegroundMgr::ScheduleQueueUpdate(uint32 arenaMatchmakerRating, uint8 arenaType, BattlegroundQueueTypeId bgQueueTypeId, BattlegroundTypeId bgTypeId)
{
    //This method must be atomic, @todo add mutex
    //we will use only 1 number created of bgTypeId
    uint64 const scheduleId = ((uint64)arenaMatchmakerRating << 32) | (uint32(arenaType) << 24) | (bgQueueTypeId << 16) | (bgTypeId << 8);
...
void BattlegroundMgr::LoadBattleMastersEntry()
{
    ...
    if (!result)
    {
        IC_LOG_INFO("server.loading", ">> Loaded 0 battlemaster entries. DB table `battlemaster_entry` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            IC_LOG_ERROR("sql.sql", "Table `battlemaster_entry` contain entry %u for not existed battleground type %u, ignored.", entry, bgTypeId);
            continue;
        }
    ...
    IC_LOG_INFO("server.loading", ">> Loaded %u battlemaster entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
