...
class EventMap
{
    ...
    */
    ...
    public:
        ...
        /**
        * @name Reset
        * @brief Removes all scheduled events and resets time.
        */
        void Reset()
        {
            _eventMap.clear();
        ...
        */
        void ScheduleEvent(uint32 eventId, uint32 time, uint32 group = 0)
        {
            if (group && group <= 8)
                eventId |= (1 << (group + 15));
        ...
        * @param group The group which the event is associated to. Has to be between 1 and 8. 0 means it has no group.
        */
        void RescheduleEvent(uint32 eventId, uint32 time, uint32 group = 0)
        {
            CancelEvent(eventId);
            ScheduleEvent(eventId, time, group);
        }
